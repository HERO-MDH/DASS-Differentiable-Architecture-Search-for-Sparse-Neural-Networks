...
from misc import xe_with_one_hot
...
def trades_loss(
    ...
    loss_robust = (1.0 / batch_size) * criterion_kl(
        F.log_softmax(model(x_adv)[0], dim=1), F.softmax(model(x_natural)[0], dim=1)
    )
...
def trades_loss_aux(
    ...
    if distance == "l_inf":
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss_kl = criterion_kl(
                    F.log_softmax(model(x_adv)[1], dim=1),
        ...
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                loss = (-1) * criterion_kl(
                    F.log_softmax(model(adv)[1], dim=1), F.softmax(model(x_natural)[1], dim=1)
                )
            loss.backward()
            # renorming gradient
            grad_norms = delta.grad.view(batch_size, -1).norm(p=2, dim=1)
    ...
    optimizer.zero_grad()
    # calculate robust loss
    logits,logits_aux = model(x_natural)
    loss_natural = natural_criterion(logits_aux, y)
    loss_robust = (1.0 / batch_size) * criterion_kl(
        F.log_softmax(model(x_adv)[1], dim=1), F.softmax(model(x_natural)[1], dim=1)
    )
    loss = loss_natural + beta * loss_robust
    return loss
