...
def main():
    args = parse_args()
    args.configs = 'configs/configs_imagenet.yml'
    parse_configs_file(args)
    ###################### for debug###################
    args.arch = 'darts_image'
    args.exp_mode = 'finetune'
    args.source_net = './trained_models/temp/finetune/latest_exp/checkpoint/model_best.pth.tar'
    args.trainer = 'base'
    args.val_method = 'base'
    args.k =0.01
    args.lr = 0.1#0.025
    # args.resume = './trained_models/temp/prune/latest_exp/checkpoint/model_best.pth.tar'
    # sanity checks
    ...
    if len(gpu_list) > 1:
        ...
        model = nn.DataParallel(
            models.__dict__[args.arch](
                cl, ll, args.init_type,args,num_classes=args.num_classes,
            ),
            gpu_list,
        ).to(device)
