...
from utils.adv import trades_loss,trades_loss_aux
...
def train(
    ...
    model.drop_path_prob = args.drop_prob_td * epoch / args.epochs_td
    for i, data in enumerate(dataloader):
        ...
        if i == 0:
            print(
                images.shape,
                target.shape,
                f"Batch_size from args: {args.batch_size_td}",
        ...
        if args.auxiliary_darts:
            loss_aux = trades_loss_aux(
            model=model,
            x_natural=images,
            y=target,
            device=device,
            ...
            clip_max=args.clip_max,
            distance=args.distance)
            loss += args.auxiliary_weight_darts * loss_aux
