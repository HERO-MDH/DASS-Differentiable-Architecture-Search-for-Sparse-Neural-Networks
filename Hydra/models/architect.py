...
class Architect(object):
  ...
  def __init__(self, model, criterion, args):
    ...
    self.network_weight_decay = args.weight_decay_darts
    
    self.model = model
    print(self.model.conv_layer,"**********************")
    self.criterion = criterion
    self.optimizer = torch.optim.Adam(self.model.arch_parameters(),
        lr=args.arch_learning_rate_darts, betas=(0.5, 0.999), weight_decay=args.arch_weight_decay_darts)
  ...
  def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
    ...
    set_prune_rate_model(unrolled_model,self.args.search_k_darts)
    unfreeze_vars(unrolled_model, "popup_scores")
    unfreeze_vars(unrolled_model, "weight")
    unfreeze_vars(unrolled_model, "bias")
